...
class ScrollCreditsWidget(QWidget):
    ...
    def _load_commit_data(self):
        ...
        try:
            ...
            log.error(f"gt(加载贡献者数据失败: ){e}")
            # 添加错误信息
            error_label = QLabel(f"gt(加载贡献者信息失败: ){str(e)}")
    ...
    def _create_contributor_widget(self, contributor):
        ...
        if contributor.get('is_category_header', False):
            ...
            name_label.setStyleSheet(f"""
                QLabel {{
                    color: {text_color}gt(;  /* 根据主题设置文字颜色 */
                    font-size: 20px;
                    font-weight: bold;
                    padding: 10px;
                    font-family: Arial, sans-serif;
        ...
        name_label.setStyleSheet(f"""
            QLabel {{
                color: {main_text_color}gt(;  /* 根据主题设置文字颜色 */
                font-size: 18px;
                font-weight: bold;
                padding: 5px;
                font-family: Arial, sans-serif;
        ...
        if 'members' in contributor and isinstance(contributor['members'], list | tuple):
            ...
            members_label.setStyleSheet(f"""
                QLabel {{
                    color: {secondary_text_color}gt(;  /* 根据主题设置文字颜色 */
                    font-size: 16px;
        ...
        if 'contributions' in contributor:
            ...
            contributions_label.setStyleSheet(f"""
                QLabel {{
                    color: {light_text_color}gt(;  /* 根据主题设置文字颜色 */
                    font-size: 14px;
                    padding: 5px;
                    font-family: Arial, sans-serif;
                }}
